...
export default function Library() {
  ...
  const filtered = libraryArticles.filter(a => {
    ...
    <AnimatedPage className="flex-1 px-4 py-12 md:px-8 max-w-5xl mx-auto w-full flex flex-col gap-10">
      ...
      <ScrollReveal>
        <div className="flex flex-col gap-6">
          <h1 className="text-4xl md:text-5xl font-heading text-primary leading-tight">Rights Library</h1>
        ...
        {recentSearches.length > 0 && (
          <div className="flex flex-wrap items-center gap-2 mt-[-10px]">
            <span className="text-sm font-heading text-on-surface-variant">Recent:</span>
      ...
      >
        {loading ? (
              ...
              <TiltCard>
                ...
                >
                  <div className="flex flex-col gap-3 items-start flex-1 pr-4">
                    <motion.span whileHover={{ y: -2 }} className="pill-badge">{article.tag}</motion.span>
                    <div>
          ...
          <motion.div variants={itemVariants} className="py-16 flex flex-col items-center text-center glass-panel border border-outline rounded-2xl">
            <h3 className="font-heading text-2xl text-on-background mb-2">No results found</h3>
            <p className="text-on-surface-variant">We couldn't find any articles matching your search.</p>
            <button 
